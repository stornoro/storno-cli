...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `uuid` | string | Yes | Product UUID to update |
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `name` | string | No | Product name |
| `code` | nullable string | No | Product code / SKU. Pass null to clear the existing code. |
| `description` | nullable string | No | Product description |
| `unitOfMeasure` | string | No | Unit of measure code (e.g. "buc", "ore", "kg") |
| `defaultPrice` | number | No | Default unit price |
| `currency` | string | No | ISO 4217 currency code |
| `vatRate` | number | No | VAT rate percentage (e.g. 19 for 19%) |
| `vatCategoryCode` | string | No | VAT category code (e.g. "S", "Z", "E") |
| `ncCode` | nullable string | No | NC (Nomenclatura Combinata) code |
| `cpvCode` | nullable string | No | CPV code for public procurement |
| `color` | nullable string | No | Optional hex colour swatch shown on the POS product grid (e.g. "#1e40af"). Six-digit hex with or without leading "#"; pass null to clear. |
| `categoryId` | nullable string | No | UUID of a product_category for POS grid grouping; pass null to clear. |
| `sgrAmount` | nullable number | No | Romanian SGR (Sistem Garantie-Returnare) deposit per unit, e.g. 0.50. Null clears. The POS auto-injects a single VAT-exempt deposit line summing all SGR product quantities. |
...
| `color` | nullable string | No | Optional hex colour ("#RRGGBB" or "RRGGBB"); invalid values are silently dropped |
| `sortOrder` | number | No | Sort order in the chip strip (smaller = earlier; default 0) |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `uuid` | string | Yes | Category UUID to update |
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `name` | string | No | Display name, 1–100 characters |
| `color` | nullable string | No | Hex colour swatch; null to clear |
| `sortOrder` | number | No | Sort order in the chip strip |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `uuid` | string | Yes | Category UUID to delete |
| `companyId` | string | No | Company UUID override (uses active company if not set) |
...
|------|------|----------|-------------|
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `type` | enum(bank, cash) | No | Account type. "bank" (default) for IBAN-backed accounts; "cash" for the physical till that backs POS / cash-register reports. |
| `iban` | string | Conditional | International Bank Account Number. Required for type=bank, optional for type=cash. |
...
| `openingBalance` | number | No | Initial cash-on-hand. Required to enable cash-register reporting on cash accounts. Once set, locks and can only be modified via forward-going cash movements. Must be >= 0. |
| `openingBalanceDate` | string | No | Date the opening balance was taken (YYYY-MM-DD). Required when openingBalance is supplied. |
...
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `type` | enum(bank, cash) | No | Account type. Switching after creation is allowed but should be avoided once movements exist. |
| `iban` | nullable string | No | New IBAN (must be valid and unique within company). Pass null to clear (only valid for cash accounts). |
...
| `openingBalance` | number | No | Initial cash-on-hand. Locks once set; correct via cash movements thereafter. |
| `openingBalanceDate` | string | No | Date the opening balance was taken (YYYY-MM-DD). Required when openingBalance is being set for the first time. |
...
|------|------|----------|-------------|
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `from` | string | No | Start date YYYY-MM-DD (defaults to the cash account opening date or 30 days ago) |
...
|------|------|----------|-------------|
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `kind` | enum(deposit, withdrawal, other) | Yes | deposit (cash leaves the till), withdrawal (cash put into the till), or other (miscellaneous; direction required) |
| `direction` | enum(in, out) | Conditional | Required only when kind=other; ignored otherwise |
| `amount` | number | Yes | Positive amount, >= 0.01 |
| `movementDate` | string | Yes | YYYY-MM-DD. Cannot be earlier than the cash account opening date. |
| `description` | string | No | Free-form note shown in the ledger |
| `documentNumber` | string | No | Reference number (e.g., bank slip / expense receipt) |
...
| `companyId` | string | No | Company UUID override (uses active company if not set) |
| `kind` | enum(deposit, withdrawal, other) | No | |
| `direction` | enum(in, out) | No | |
| `amount` | number | No | |
| `movementDate` | string | No | YYYY-MM-DD |
| `description` | nullable string | No | Pass null to clear |
| `documentNumber` | nullable string | No | Pass null to clear |
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `uuid` | string | Yes | UUID of the parent issued receipt to refund |
| `companyId` | string | No | Company UUID (overrides configured default) |
| `lineSelections` | array of `{ sourceLineId: string, quantity: string\|number }` | No | When provided, only the listed lines are refunded with the given (partial) quantities. When omitted, the entire receipt is refunded. |
...
| `dosare_billing` | the tenant's invoices: recurring invoice, issued invoices with paid / unpaid / overdue state, invoices received from the tenant, investment compensation balance |
