/**
 * McpServer factory — creates and configures a Storno MCP server instance.
 *
 * Extracted from index.ts so both stdio and HTTP transports can reuse it.
 * In HTTP stateful mode a fresh McpServer is created per session.
 */
...
import { tools as productCategoriesTools } from './tools/product-categories.js';
...
import { tools as cashRegisterTools } from './tools/cash-register.js';
...
/** Auto-login if email/password are set but no token exists yet. */
